...
function TaskItem({ text, onUpdate, onMoveUp, onMoveDown, canMoveUp, canMoveDown }) {
  ...
  return (
    <div className="flex items-center gap-2 py-1 group">
      ...
      <input
        type="checkbox"
        checked={!!onUpdate.completed}
        onChange={onUpdate.toggleComplete}
        className="mt-1"
      />
      ...
      <div className="flex-1 min-w-0">
        {isEditing ? (
          ...
          <button
            ...
            className={`w-full text-left text-sm rounded px-2 py-1 -mx-2 ${onUpdate.completed
      ...
      {/* Move buttons unchanged */}
      <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button
          type="button"
          onClick={onMoveUp}
          disabled={!canMoveUp}
          className="p-1 rounded text-gray-600 hover:bg-gray-100 disabled:opacity-30"
        >
          ↑
        </button>
        <button
